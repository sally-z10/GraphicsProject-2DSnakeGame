...
{
    glColor3f(0.8, 0.2, 0.4); // Dark red or any color that fits the Harry Potter theme

    // Use circles instead of squares for the body parts (this adds a quirky touch)
    float radius = 0.45;
    ...
    for (int i = 0; i < num_segments; i++) {
        ...
        glVertex2f(x + 0.5 + dx, y + 0.5 + dy); // Specify the vertex
    }
    glEnd();
}
